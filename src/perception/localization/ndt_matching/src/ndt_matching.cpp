...
void NdtMatching::PointsCallback_(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
  ...
  if (map_loaded_ == 1 && init_pos_set_ == 1)
  {
    ...
    // double pose_diff = sqrt((ndt_pose.position.x - predict_pose_.position.x) * (ndt_pose.position.x - predict_pose_.position.x) +
    //                         (ndt_pose.position.y - predict_pose_.position.y) * (ndt_pose.position.y - predict_pose_.position.y) +
    //                         (ndt_pose.position.z - predict_pose_.position.z) * (ndt_pose.position.z - predict_pose_.position.z));
    
    // if(pose_diff <= UPDATE_POSE_THRESHOLD) predict_pose_ = ndt_pose;
  }
